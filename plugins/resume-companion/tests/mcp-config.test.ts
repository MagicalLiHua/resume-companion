...
describe('Codex MCP policy', () => {
  test('keeps the profile service required and applies least-privilege browser defaults', async () => {
    ...
    const browser = manifest.mcpServers.chrome_devtools;
    ...
    const automatic = ['list_pages', 'select_page', 'take_snapshot', 'wait_for', 'list_network_requests', 'list_console_messages', 'get_console_message', 'take_screenshot', 'fill_form', 'fill', 'click', 'hover'];
