...
const browserTools = [
  ...
  ['act', 'act', '执行一个或一批表单基础动作', '使用观察返回的引用、快照和值 token 执行输入、选择、点击或批量写入。同一快照内的独立文本框、复选框、单选项和原生下拉应合并为一次 set_values（最多 20 项），并直接利用回执中的变化观察，避免逐项往返。最终提交、声明、验证、删除和上传由用户处理。', false],
