...
Resume Companion combines a local multi-profile library with a constrained Chrome DevTools browser driver. The default driver connects to the user's current Chrome and keeps existing website sessions. An optional extension driver is only a compatibility fallback. A separate model API or business backend is unnecessary.
...
For webpage work, inspect `browser.ready`, `browser.profile_mode`, `browser.permission_state` and `browser.connected`. The browser connection is lazy: call `resume_list_tabs` to start it when ready but disconnected. In `auto_connect` mode, if Chrome requests remote-debugging access, tell the user to click **Allow** in Chrome and then retry `resume_list_tabs` once. Do not request macOS permission to modify applications; Resume Companion does not need it.
