...
- **L0 independent**: writing one target cannot reveal, replace, disable or clear another target. Batch these with `fill_form`.
- **L1 trigger**: a country, answer, employment status or similar choice changes later fields. Handle it separately, then reuse the returned snapshot.
- **L2 dynamic candidate**: the target exists only after opening, searching or expanding. Use one matching recipe below.
- **L3 record boundary**: adding or saving a record, changing a step or opening a new editor can rebuild the page. Discard old UIDs afterwards.
...
**Do:** skip an already-correct value; preserve a draft of unclear origin; batch independent controls. Use the format the page exposes for date and number fields.
...
**Do:** reason about the option meaning and the full group state. Batch only when selecting one option cannot replace the rest of the section.
...
**Do:** click the trigger with an attached snapshot, identify the option in the current popup, then click the actual displayed option.
...
**Do:** open if required; fill the smallest distinctive search term; wait for a known candidate or an explicit no-results state; click only a candidate shown by the page.
...
**Do:** open the control; choose one level; reuse or refresh the snapshot; repeat until the intended leaf. Select only one level per structural change.
...
**Do:** expand only nodes on the target path; after each expansion, locate the next child in the new snapshot; choose a leaf unless the page explicitly permits parent selection.
...
**Do:** prefer direct semantic filling for native controls. For ranges, fill start before end. For custom calendars, navigate one bounded unit at a time and use current labels rather than coordinates.
...
**Do:** open the dialog; operate only inside its visible scope; use one batch for independent options; click the dialog's explicit confirmation.

**DOM boundary:** closing the dialog invalidates all dialog UIDs and may rebuild the parent field.
...
**Do:** work on exactly one record; identify it by section and stable visible facts; complete its independent fields; save the record; reobserve the rebuilt records list before the next record.

**DOM boundary:** add, edit, save, cancel and delete are L3 boundaries. Discard all old editor and record UIDs.
...
**Do:** activate the disclosure; wait for a specific section label or target field; reuse the wait snapshot.
...
**Do:** for same-origin content already exposed by the official tools, use its current UIDs normally. Keep parent and frame fields in separate batches when structure may change.
