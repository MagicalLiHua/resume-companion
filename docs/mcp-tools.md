...
0.11.1 把资料与浏览器分成两个 MCP。Agent 直接调用官方 Chrome DevTools MCP，不再经过 Resume Companion 的浏览器代理协议。
...
截图用于理解布局，操作仍使用 UID。0.11.1 不启用实验性 `click_at`。
...
逐工具审批按工具名生效，不能判断某个具体按钮是不是最终提交。最终提交边界仍依赖 skill、页面证据和人工监督。0.11.1 不宣传参数级强制防误投递。
