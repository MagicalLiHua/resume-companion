import { PROTOCOL_VERSION, canonical, type ObserveParams, type Scalar } from '../../../../plugins/resume-companion/src/protocol';
...
export class Observer {
  ...
  observe(params: ObserveParams = {}, extra: Record<string, unknown> = {}) {
    ...
    if (params.cursor) {
      ...
      snapshot = { id: crypto.randomUUID(), ...current, order: [...current.entries.keys()], created: Date.now(), ...(params.scope_ref ? { scopeRef: params.scope_ref } : {}), exposed: new Set<string>() };
    ...
    const response: Record<string, any> = { protocol_version: PROTOCOL_VERSION, driver: 'extension', session_id: this.session, snapshot_id: snapshot.id,
    ...
    if (response.truncated) { const cursor = crypto.randomUUID(); this.cursors.set(cursor, { snapshot: snapshot.id, offset: next, mode, ...(params.scope_ref ? { scope: params.scope_ref } : {}) }); response.next_cursor = cursor; }
