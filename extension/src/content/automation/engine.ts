import { z } from 'zod';
import { canonical, createAutomationSchemas, type ActParams, type Action, type Condition, type ObserveParams, type Scalar, type UndoParams, type WaitParams, type Write } from '../../../../plugins/resume-companion/protocol';
...
export class AutomationEngine {
  ...
  private async execute(action: Action, snapshot: string, record: RecordEntry, guard: () => void, mark: () => void): Promise<Record<string, any> & { status: Status }> {
    ...
    if (action.kind === 'press_key') {
      mark(); node.focus({ preventScroll: true }); node.dispatchEvent(new KeyboardEvent('keydown', { key: action.key, bubbles: true, cancelable: true })); node.dispatchEvent(new KeyboardEvent('keyup', { key: action.key, bubbles: true }));
      await pause(100); guard();
      return { status: 'dispatched', ref: entry.ref, note: '已派发合成按键；原生默认动作与 isTrusted 行为不保证支持，请依据观察继续' };
    }
  ...
  private async write(action: Write, entry: Entry, snapshot: string, record: RecordEntry, guard: () => void, mark: () => void) {
    ...
    } else if (node.matches('[role="checkbox"],[role="radio"]') && !editable(node)) { if (readValue(node) !== value) node.click(); }
