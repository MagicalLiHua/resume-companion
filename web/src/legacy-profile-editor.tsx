...
import { newId, newRecord, parseProfile, type Profile, type RecordSection } from '../../extension/src/domain/profile-v1';
...
function Input({ label, value, onChange, type = 'text', multiline = false, placeholder }: { label: string; value: string | null; onChange: (s: string | null) => void; type?: string; multiline?: boolean; placeholder?: string }) {
  const id = useId();
...
function Select({ label, value, options, onChange }: { label: string; value: string | null; options: [string, string][]; onChange: (s: string | null) => void }) {
  const id = useId();
...
export function importDiff(before: Profile, after: Profile) {
  function flatten(value: unknown, path: string[] = [], out: Record<string, string> = {}) {
    if (value && typeof value === 'object') {
      for (const [k, v] of Object.entries(value)) if (!['id', 'profile_id', 'revision', 'schema_version'].includes(k)) flatten(v, [...path, /^\d+$/.test(k) ? `第 ${Number(k) + 1} 项` : friendly[k] ?? k], out);
...
export function ProfileFields({profile, onChange, tab, disabled = false}: {profile: Profile; onChange: (p: Profile) => void; tab: Tab; disabled?: boolean}) {
  ...
  function recordFields(section: RecordSection, i: number) {
    if (section === 'education') {
      ...
      return <><Input label="学校名称" value={r.school} onChange={v => changeRecord(section, i, 'school', v)}/><Input label="所学专业" value={r.major} onChange={v => changeRecord(section, i, 'major', v)}/><Select label="就读学历" value={r.education_level} options={[["associate","大专"],["bachelor","本科"],["master","硕士研究生"],["doctor","博士研究生"],["other","其他"]]} onChange={v => changeRecord(section, i, 'education_level', v)}/><Select label="学习形式" value={r.study_mode} options={[["full_time","全日制"],["part_time","非全日制"],["other","其他"]]} onChange={v => changeRecord(section, i, 'study_mode', v)}/><Input label="已获得学位" placeholder="尚未授予则留空" value={r.degree} onChange={v => changeRecord(section, i, 'degree', v)}/><Input label="预计获得学位" placeholder="例如：工学硕士" value={r.expected_degree} onChange={v => changeRecord(section, i, 'expected_degree', v)}/>{dates(section, i)}<div className="toggles span-2"><label><input type="checkbox" checked={r.completed} onChange={e => edit(p => { p.education[i].completed = e.target.checked; if (e.target.checked) { p.education[i].is_current = false; p.education[i].is_expected_end = false; } })}/>已完成这段教育</label><label><input type="checkbox" checked={r.is_current} disabled={r.completed} onChange={e => changeRecord(section, i, 'is_current', e.target.checked)}/>目前在读</label><label><input type="checkbox" checked={r.is_expected_end} disabled={r.completed} onChange={e => changeRecord(section, i, 'is_expected_end', e.target.checked)}/>结束时间为预计</label></div></>;
    }
    if (section === 'experience') {
      const r = profile.experience[i];
      return <><Input label="单位名称" value={r.organization} onChange={v => changeRecord(section, i, 'organization', v)}/><Input label="担任职位" value={r.role} onChange={v => changeRecord(section, i, 'role', v)}/><Select label="经历类型" value={r.kind} options={[["internship","实习"],["work","正式工作"]]} onChange={v => changeRecord(section, i, 'kind', v ?? 'internship')}/><div/>{dates(section, i)}<div className="toggles span-2"><label><input type="checkbox" checked={r.is_current} onChange={e => changeRecord(section, i, 'is_current', e.target.checked)}/>目前仍在此任职</label></div><Input label="工作内容（每行一项）" multiline value={r.facts.map(f => f.text).join('\n')} onChange={v => changeFacts(section, i, v)}/></>;
    }
    if (section === 'projects') {
      const r = profile.projects[i];
      return <><Input label="项目名称" value={r.name} onChange={v => changeRecord(section, i, 'name', v)}/><Input label="项目角色" value={r.role} onChange={v => changeRecord(section, i, 'role', v)}/>{dates(section, i)}<div className="toggles span-2"><label><input type="checkbox" checked={r.is_current} onChange={e => changeRecord(section, i, 'is_current', e.target.checked)}/>项目仍在进行</label></div><Input label="技术栈（用顿号或逗号分隔）" value={r.technologies.join('、')} onChange={v => changeRecord(section, i, 'technologies', v ? v.split(/[、,，]/) : [])}/><div/><Input label="项目描述（每行一项）" multiline value={r.facts.map(f => f.text).join('\n')} onChange={v => changeFacts(section, i, v)}/></>;
    }
